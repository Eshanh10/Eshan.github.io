...
var FormTranslation = {
  ...
  init: function (properties) {
    ...
    this.getTranslations(languages, function() {
      ...
      if (!self.urlParams.language) { // url parameter should be prioritized
        if (self.properties.saveUserLanguage && savedLang && typeof self.translations[savedLang] !== 'undefined') {
        ...
        if (typeof self.dictionary[self.to] === 'undefined') {
          self.getTranslations([self.to], function() { self.actionTranslation(); });
        } else {
          self.actionTranslation();
        }
        JotForm.runAllCalculations();
  ...
  getFormElements: function() {
    ...
    var cardProgressItems = $$('.cardProgress-seeAll, .cardProgress-middleText', '.cardProgress-goBack');
  ...
  simplifyfWord: function (word){
    var simplified = word.replace(/^\s+|\s+$/g, '').strip();
    simplified = simplified.replace(/&gt;/g, '>').strip();
    simplified = simplified.replace(/&lt;/g, '<').strip();
    simplified = simplified.replace(/&emsp;/g, ' ').strip();   
    simplified = simplified.replace(/<br>/g, ' ').strip();
    simplified = simplified.replace(/\s+/gim, ' ').strip();
    simplified = simplified.replace(/&amp;/g, '&').strip();
    return simplified;
  },
  translate: function (word, compoundHint) {
    ...
    if (tmpWord !== nbspReplaced) {
      ...
      for (var language in self.languages){
        if (language != self.to){
          for (var searchedWord in self.dictionary[language]){
            var compareWord = self.dictionary[language][searchedWord];
  ...
  translateFormElements: function(formElementList){
    ...
    $A(formElementList).each(function(e){
      ...
      } else if (!e.querySelector('.replaceTag')) { // if there is a replace tag, do not translate it
        if (e.children.length > 0) {
    ...
    if (iframes.length > 0) {
      for (var i = 0; i < iframes.length; ++i) {
        if (iframes[i].hasAttribute('id') && iframes[i].id.indexOf('customFieldFrame') !== -1) {
          ...
          self.sendWidgetMessage(JSON.stringify(msg), iframes[i].id);
        }
      }
    }
  ...
  clickEventForDropDown: function(){
    ...
    $('langDd').setAttribute('role','listbox');
    ...
    if (typeof self.properties.showStatus != 'undefined') {
      if (self.properties.showStatus == 'Flag & Text') {
        ...
        */
        ...
        $$('#langDd div')[0].update('');
        $$('div[id="langDd"]')[0].setStyle({ 'width': '50px' });
